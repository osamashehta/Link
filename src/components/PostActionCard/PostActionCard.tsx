...
const PostActionCard = ({postId, token, setShowEditPost, showEditPost, isOwner}: {postId: string, token: string, setShowEditPost: (showEditPost: boolean) => void, showEditPost: boolean, isOwner: boolean}    ) => {
  ...
  const {mutate} = useMutation({
    mutationFn: (id: string) => apiServiceCall({
      endPoint: `posts/${id}`,
      method: "DELETE",
      headers: {
        token: token
...
<div
 ...
 <>
  <div onClick={() => setReportReason(true)}
                className="flex items-center justify-start gap-2 cursor-pointer"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#808080" d="M12 16.423q.262 0 .439-.177q.176-.177.176-.438t-.177-.439t-.438-.177t-.438.177t-.177.439t.177.438t.438.177m-.5-2.961h1V7.384h-1zM8.673 20L4 15.336V8.673L8.664 4h6.663L20 8.664v6.663L15.336 20zm.427-1h5.8l4.1-4.1V9.1L14.9 5H9.1L5 9.1v5.8zm2.9-7"/></svg>
                <p>Report</p>
              </div>
            <ReportModal reportReason={reportReason} setReportReason={setReportReason}/>
 </>
